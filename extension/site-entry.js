...
(() => {
  ...
  const panel=document.createElement("section");panel.className="panel";panel.setAttribute("aria-label","SCUT 课堂助手");
  ...
  panel.querySelector(".close").onclick=()=>expanded(false);dock.onclick=()=>expanded(true);
  ...
  async function update(){
    ...
    try{const state=await send("PAGE_ASSISTANT_READY");
      ...
      expanded(state.autoOpen&&!state.popupOpened);
    }catch(e){panel.querySelector("h3").textContent="课堂助手";panel.querySelector(".description").textContent=e.message;}
  }
